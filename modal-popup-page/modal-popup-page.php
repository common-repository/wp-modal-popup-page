...
Plugin Name: WP Modal Popup Page

Plugin Script: modal-popup-page.php

Plugin URI: http://ebook-of-success.com

Description: Cofigurable popup page module. It works also if WP Super Cache or W3 Total Cache is enabled!
...
Author URI: http://ebook-of-success.com
...
This program is not for commercial purpose; only the author can redistribute it and/or modify

it. You cannot redistribute it and/or modify it under the terms of the Attribution-NonCommercial-NoDerivs 3.0 Unported, as published by

the Creative Commons Organization except in the strict case in which the author, in this case Kossi Afedo, gives you the right to.
...
$smp_plugin_url_base = WP_PLUGIN_URL . '/modal-popup-page';
...
<script type="text/javascript">
	...
	jQuery(document).ready(function($) {
		...
		$(document).ready(function(){
			...
			var smp_cookie_name_c = 'smp_<?php echo $options['cookie_id']?>_c';//3

			var smp_cookie_name_d = 'smp_<?php echo $options['cookie_id']?>_d';//4
			...
			function smp_reset_cookies(){
				...
				c_value_d = $.cookie(smp_cookie_name_d);
			...
			$(document).ready(function(){
				...
				if (!smp_reset_cookies()){
					...
					} else if (smp_show_mode == 3){

						date.setTime(date.getTime() + (<?php echo $options['cookie_duration_sec']?> * 1000));

						c_value = c_value_c;

						smp_cookie_name = smp_cookie_name_c;

					} else if (smp_show_mode == 4){
...
function smp_create_menu() {
	...
	$page = add_menu_page('Popup Modal', 'Popup Modal', 'administrator', __FILE__, 'smp_settings_page',plugins_url('/images/popup_icon.gif', __FILE__),'59.5');
...
<div class="wrap">
	...
	<h2>WP Modal Popup Page 1.0</h2>

	<div style="padding-bottom:10px;margin-top:5px;margin-bottom:10px;">

	by <strong><a target="_blank" href="http://ebook-of-success.com">Kossi Afedo</a></strong>	
...
<form id="target" method="post" action="options.php">
    ...
    <table class="form-table">
        ...
        <tr valign="top">
        	...
        	<th scope="row"><strong>Show the popup (if not disabled, a cookie will trigger if the popup will show or not for the same client on return to that page):</strong></th>
        	...
        	<td>
        		...
        		<input type="radio" <?php echo($options['show_mode']==2?'checked':'')?> name="smp-options[show_mode]" value="2"> For the first <input size="5" type="text" name="smp-options[cookie_num_visits]" value="<?php echo $options['cookie_num_visits']; ?>" /> visits<br/>

        		

        		<input type="radio" <?php echo($options['show_mode']==3?'checked':'')?> name="smp-options[show_mode]" value="3"> Every <input size="5" type="text" name="smp-options[cookie_duration_sec]" value="<?php echo $options['cookie_duration_sec']; ?>" /> seconds<br/>

        		

        		<input type="radio" <?php echo($options['show_mode']==4?'checked':'')?> name="smp-options[show_mode]" value="4">Disabled (no cookies - every time when the page is accesed by the same clients, it pops out.
    ...
    <table class="form-table">
        ...
        <td><input type="radio" <?php echo($options['overlay_close']=='true'?'checked':'')?> name="smp-options[overlay_close]" value="true">Yes
          ...
          <td align="right"><a href="http://bitly.com/pluginspopup" target="_blank"><img src="http://ebook-of-success.com/membersarea/affiliate/banners/banner250x250.jpg" /></a></td>
        </tr>         
